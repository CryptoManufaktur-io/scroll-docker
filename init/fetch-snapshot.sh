...
# shellcheck disable=SC2154
  ...
  if [[ "${filename}" =~ \.tar\.zst$ ]]; then
    pzstd -c -d "${filename}" | tar xvf - -C "${__datadir}"/geth
  elif [[ "${filename}" =~ \.tar\.gz$ || "${filename}" =~ \.tgz$ ]]; then
    tar xzvf "${filename}" -C "${__datadir}"/geth
  elif [[ "${filename}" =~ \.tar$ ]]; then
    tar xvf "${filename}" -C "${__datadir}"/geth
  elif [[ "${filename}" =~ \.lz4$ ]]; then
    lz4 -d "${filename}" | tar xvf - -C "${__datadir}"/geth
  else
    __dont_rm=1
